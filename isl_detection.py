...
parser = argparse.ArgumentParser(description='Indian Sign Language Detection')
...
# For webcam input or video file:
input_source = args.input
# Check if the input is a camera index or a video file
if input_source.isdigit():
    cap = cv2.VideoCapture(int(input_source))
else:
    cap = cv2.VideoCapture(input_source)

# Check if camera or video file opened successfully
if not cap.isOpened():
...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    # Flip the image horizontally only if using a webcam
    if input_source.isdigit():
